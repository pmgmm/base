...
function ready() {
    ...
    if (!obj_filter_helper.applyStored()) {
        // Ordenação default
        obj_reader_helper.addOrder('sp.code', 'ASC');
        applyFilter(); 
    }
...
function confirmDeleteRecord(id, name) {
    obj_askbox.show =  {
        color: 'blue',
        title: TranslationHelper.translate('Eliminar permissão'),
        question: TranslationHelper.translate('Confirma a eliminação da permissão') + '<br><b>' + name + '</b>',
        actions: [
...
function confirmDeleteBulk() {
    ...
    if (ids.length > 0) {
        obj_askbox.show = {
            color: 'blue',
            title: TranslationHelper.translate('Eliminar permissões'),
            question: TranslationHelper.translate('Confirma a eliminação das permissões selecionadas?'),
            actions: [
                {caption: TranslationHelper.translate('Não'), color: "white"},
 ...
 */
 function resetFilterComponents() {
    ...
    FormHelper.setComponentValue('permission_type', '');
 }
...
function setFilter() {
    ...
    obj_reader_helper.addField('sp.type', FormHelper.getComponentValue('permission_type'));
}
 ...
 */
 function populateComponents(filter) {
    ...
    FormHelper.setComponentValue('permission_type', filter.fields['sp.type']);
}
...
function applyFilter() {
    ...
    var reader = {ajax: obj_ajax_helper, filter: obj_reader_helper, fully_qualified_class_name: '\\CORE\\entities\\permission\\processors\\Base', action: 'get_list'};
    obj_list.fill({
        reader: reader,
    });
