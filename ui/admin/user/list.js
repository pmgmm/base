...
function ready() {
    ...
    if (!obj_filter_helper.applyStored()) {
        // Ordenação default
        obj_reader_helper.addOrder('su.name', 'ASC');
        applyFilter(); 
    }
...
function confirmDeleteRecord(id, name) {
    obj_askbox.show =  {
        color: 'blue',
        title: TranslationHelper.translate('Eliminar utilizador'),
        question: TranslationHelper.translate('Confirma a eliminação do utilizador:') + '<br><b>' + name + '</b>',
        actions: [
...
function confirmDeleteBulk() {
    ...
    if (ids.length > 0) {
        obj_askbox.show = {
            color: 'blue',
            title: TranslationHelper.translate('Eliminar utilizadores'),
            question: TranslationHelper.translate('Confirma a eliminação dos utilizadores selecionados?'),
            actions: [
                {caption: TranslationHelper.translate('Não'), color: "white"},
 ...
 */
 function resetFilterComponents() {
    ...
    FormHelper.setComponentValue('user_system', '');
    FormHelper.setComponentValue('user_active', '');
 }
...
function setFilter() {
    ...
    obj_reader_helper.addField('su._system', FormHelper.getComponentValue('user_system'));
    obj_reader_helper.addField('su.active', FormHelper.getComponentValue('user_active'));
}
 ...
 */
 function populateComponents(filter) {
    ...
    FormHelper.setComponentValue('user_system', filter.fields['su._system']);
    FormHelper.setComponentValue('user_active', filter.fields['su.active']);
}
...
function applyFilter() {
    ...
    var reader = {ajax: obj_ajax_helper, filter: obj_reader_helper, fully_qualified_class_name: '\\CORE\\entities\\user\\processors\\Base', action: 'get_list'};
    obj_list.fill({
        reader: reader,
    });
