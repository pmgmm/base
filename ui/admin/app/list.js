...
function ready() {
    ...
    if (!obj_filter_helper.applyStored()) {
        // Ordenação default
        obj_reader_helper.addOrder('sa.name', 'ASC');
        applyFilter(); 
    }
...
function confirmDeleteRecord(id, name) {
    obj_askbox.show =  {
        color: 'blue',
        title: TranslationHelper.translate('Eliminar app/pwa'),
        question: TranslationHelper.translate('Confirma a eliminação da app/pwa:') + '<br><b>' + name + '</b>',
        actions: [
...
function confirmDeleteBulk() {
    ...
    if (ids.length > 0) {
        obj_askbox.show = {
            color: 'blue',
            title: TranslationHelper.translate('Eliminar apps/pwas'),
            question: TranslationHelper.translate('Confirma a eliminação das apps/pwas selecionados?'),
            actions: [
                {caption: TranslationHelper.translate('Não'), color: "white"},
...
function deleteRecords(ids, bulk) {
    ...
    obj_ajax_helper.call({
        fully_qualified_class_name: '\\CORE\\entities\\app\\processors\\Base',
        action: action, 
        action_data: data,
 ...
 */
 function resetFilterComponents() {
    ...
    FormHelper.setComponentValue('app_active', '');
 }
...
function setFilter() {
    ...
    obj_reader_helper.addField('sa.active', FormHelper.getComponentValue('app_active'));
}
 ...
 */
 function populateComponents(filter) {
    ...
    FormHelper.setComponentValue('app_active', filter.fields['sa.active']);
}
...
function applyFilter() {
    ...
    var reader = {ajax: obj_ajax_helper, filter: obj_reader_helper, fully_qualified_class_name: '\\CORE\\entities\\app\\processors\\Base', action: 'get_list'};
    obj_list.fill({
        reader: reader,
    });
