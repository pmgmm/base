...
var list_id = 'delivery_users_list';
...
function ready() {
    ...
    if (!obj_filter_helper.applyStored()) {
        // Ordenação default
        obj_reader_helper.addOrder('du.name', 'ASC');
        applyFilter(); 
    }
...
function setFilter() {
    ...
    obj_reader_helper.addField('du.active', FormHelper.getComponentValue('user_active'));
}
 ...
 */
 function populateComponents(filter) {
    ...
    FormHelper.setComponentValue('user_active', filter.fields['du.active']);
}
...
function applyFilter() {
    ...
    var reader = {ajax: obj_ajax_helper, filter: obj_reader_helper, fully_qualified_class_name: '\\CORE\\entities\\delivery_user\\processors\\Base', action: 'get_list'};
    obj_list.fill({
        reader: reader,
    });
