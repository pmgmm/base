<?php
/**
 * UI - LISTA DE UTILIZADORES DE DELIVERY
 *  
...
final class _list {
    ...
    public function run(): void {
        try {
            ...
            $obj_page->setSubTitle(TH::translate('Estafetas'));
            $obj_page->setCurrentMenu('delivery');
            $obj_page->setBreadcrumb(TH::translate('ESTAFETAS'), true);
            ...
            // Recebe permissões  
            $permissions = PermissionHelper::validateList(array('ADMINISTRATION', 'SUPPORT', 'DELIVERY_USER'));
            ...
            $table_columns = array();
            $table_columns['du.name'] = array('title' => TH::translate('NOME'), 'width' => 230, 'order' => true);
            $table_columns['du.user'] = array('title' => TH::translate('UTILIZADOR'), 'width' => 140);
            $table_columns['du.email'] = array('title' => TH::translate('EMAIL'), 'width' => 230);
            $table_columns['du.active'] = array('title' => TH::translate('ATIVO'), 'width' => 60,'align' => 'center');
            $table_columns['_actions'] = array('width' => 180);

            // Array de filtros para slot
            $obj_filter = new Filter($repository);
            $db_fields = array('id', 'name', 'public', 'structure'); 
            $db_filter = array('list' => 'delivery_users_list', 'user_id' => array($_SESSION['USER']['id'], null), 'active' => true);
            ...
            <!-- Componente table -->
            <fwk-table id="delivery_users_list" disable="false" color="gray">
