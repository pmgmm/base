...
function ready() {
    ...
    if (!obj_filter_helper.applyStored()) {
        // Ordenação default
        obj_reader_helper.addOrder('st.source', 'DESC');
        obj_reader_helper.addOrder('st.layer', 'ASC');
        applyFilter(); 
    }
...
/**
 * Carrega página de registo em modo de update
 * 
 ...
 * 
 * @param int id = Id do registo
 * @param string source = Source identificador do registo
 * 
 * @return void
 * 
 * @pmonteiro (yyyy-mm-dd)
 */
function confirmDeleteRecord(id, source) {
    obj_askbox.show =  {
        color: 'blue',
        title: TranslationHelper.translate('Eliminar tradução'),
        question: TranslationHelper.translate('Confirma a eliminação da tradução:') + '<br><b>' + source + '</b>',
        actions: [
...
function confirmDeleteBulk() {
    ...
    if (ids.length > 0) {
        obj_askbox.show =  {
            color: 'blue',
            title: TranslationHelper.translate('Eliminar traduções'),
            question: TranslationHelper.translate('Confirma a eliminação das traduções selecionadas?'),
            actions: [
                {caption: TranslationHelper.translate('Não'), color: "white"},
 ...
 */
 function resetFilterComponents() {
    ...
    FormHelper.setComponentValue('filter_layer', '');
 }
...
function setFilter() {
    ...
    obj_reader_helper.addField('st.layer', FormHelper.getComponentValue('filter_layer'));
}
...
function populateComponents(filter) {
    ...
    FormHelper.setComponentValue('filter_layer', filter.fields['st.layer']);
}
...
function applyFilter() {
    ...
    var reader = {ajax: obj_ajax_helper, filter: obj_reader_helper, fully_qualified_class_name: '\\DEV\\entities\\translation\\processors\\Base', action: 'get_list'};
    obj_list.fill({
        reader: reader,
    });
