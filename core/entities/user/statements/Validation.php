<?php
/**
 * ENTITIES - USER - STATEMENT - VALIDATION
 * 
 * Disponibiliza métodos = statements que gerem validações de entidade USER
 *  
 * @pmonteiro (yyyy-mm-dd)
 */

namespace CORE\entities\user\statements;
...
class Validation implements \FWK\interfaces\StatementInterface {
    ...
    private function uniqueUser(): string {
        try {
            ...
            $statement = 'SELECT id FROM sys_users WHERE (user = :user) AND id != :id LIMIT 1';
    ...
    private function uniqueName(): string {
        try {
            ...
            $statement = 'SELECT id FROM sys_users WHERE (name = :name) AND id != :id LIMIT 1';
    ...
    private function uniqueEmail(): string {
        try {
            ...
            $statement = 'SELECT id FROM sys_users WHERE (email = :email) AND id != :id LIMIT 1';
