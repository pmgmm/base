...
namespace DEV\entities\translation\statements;
...
class Validation implements \FWK\interfaces\StatementInterface {
    ...
    public function getStatement(string $key, ?array $parameters = null): string {
        try{
            ...
            switch ($key) {
                case self::UNIQUE_SOURCE:
                    $statement = $this->uniqueSource();
                    break;
                default:
                    $statement = '';
                    break;
     ...
     * @pmonteiro (yyyy-mm-dd)
     */
    private function uniqueSource(): string {
        try {
              
            $statement = 'SELECT id FROM sys_translations WHERE (source = :source) AND id != :id LIMIT 1';
