...
(function() {
    ...
    <style>
        ...
        .container.gray {
            ...
            --table-column-border-color: var(--gray-light);
        ...
        .container.blue {
            ...
            --table-column-border-color: var(--blue-light);
        ...
        .container .table .header .column.bulk_action {
            width: 28px;
            justify-content: center;
        }
        .container.disable .table .header .column.bulk_action {
            display: none;
        }
        ...
        .container .table .body .row .column:not(.bulk_action):not(.actions):not(.boolean) {
        ...
        /* coluna bulk */
        .container .table .body .row .column.bulk_action {
            width: 28px;
            align-items: center;
            justify-content: center;
        }
        .container.disable .table .body .row .column.bulk_action {
            display: none;
        }
        /* coluna acções */
        .container .table .body .row .column.actions {
            white-space: nowrap;
            align-items: center;
            padding: 0px 8px; 
        }
        .container.disable .table .body .row .column.actions {
            display: none;
        }
        .container .table .body .row .column.actions div {
            padding: 0px 4px; 
        }
        /* linha checkbox */
        ...
        .container .table .body .row .column.boolean svg {
            fill: var(--color);
    ...
    class FWK_table extends HTMLElement {
        ...
        constructor() {
            ...
            this.custom_actions = this.container.querySelector('.top_actions .custom');
            this.bulk_action = this.container.querySelector('.top_actions .bulk');
        ...
        load(data) {
            ...
            if (data['records'].length) {
                ...
                let base_row_checkbox = document.createElement('fwk-checkbox');
                base_row_checkbox.className = 'action';
                base_row_checkbox.setAttribute('color', this.current_color);
                ...
                for (var i = 0; i < data['records'].length; i++) {
                    ...
                    if (this.config.table.bulk) {
                        ...
                        cell.classList.add('column', 'bulk_action');
                        let row_checkbox = base_row_checkbox.cloneNode(true);
                        // Id gerado com valor "limpo" de caracteres especiais
                        row_checkbox.id = this.id + ('_chk_' + content.key).replace(/[^A-Za-z0-9_-]/g,'_');
                        row_checkbox.setAttribute('permissions', JSON.stringify(content._permissions));
                        cell.appendChild(row_checkbox);
                    ...
                    // Atributos extra
                    if (content.values._data) {
                        row.setAttribute('data-content', JSON.stringify(content.values._data));
                    } 

                    // Itera colunas da linha
                    for (var [key, column] of Object.entries(this.config.columns)) {
        ...
        connectedCallback() {
            ...
            let custom_actions  = this.custom_actions;
            ...
            bulk_action.setAttribute('value', this.language.bulk_actions);
            ...
            if (component.hasAttribute('color') && ['blue'].includes(component.getAttribute('color'))) {
                ...
                this.current_color = component.getAttribute('color');
                this.filter_action.setAttribute('color', 'gray');
                this.bulk_action.setAttribute('color', 'gray');
            ...
            slots_config.addEventListener('slotchange', evt => {
                ...
                if (!this.slots_config_processed) {
                    ...
                    if (config.actions) {
                        custom_actions.classList.remove('hide');
                    }

                    // Acções bulk
                    config.bulk = content.bulk;
                    if (config.bulk) {
                        bulk_action.classList.remove('hide');
                    }
            ...
            slots_config_columns.addEventListener('slotchange', evt =>  {
                ...
                if (!this.slots_config_columns_processed) {
                    ...
                    if (component.config.table.bulk) {
                        let cell = document.createElement('div');
                        cell.classList.add('column');
                        cell.classList.add('bulk_action');
                        let allrows_checkbox = document.createElement('fwk-checkbox');
                        allrows_checkbox.id = component.id + '_all';
                        allrows_checkbox.setAttribute('color', 'white');

                        // Ouvir click na checkbox
                        allrows_checkbox.addEventListener('click', evt =>  {
                            if (allrows_checkbox.value) { // Seleccionar todas as linhas carregadas
                                component.select();
                        ...
                        cell.appendChild(allrows_checkbox);
