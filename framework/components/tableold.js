...
(function() {
    ...
    class FWK_table extends HTMLElement {
        ...
        data(key) {
            ...
            if (this.component.getElementById(this.id + '_' + key + '_data')) {
                data = JSON.parse(this.component.getElementById(this.id + '_' + key + '_data').innerHTML);
            }
            return data;
        }
            ...
            if (data['records'].length) {
                ...
                for (var i = 0; i < data['records'].length; i++) {
                    ...
                    if (this.config.table.bulk) {
                        ...
                        fwk_checkbox.id = this.id + ('_' + content.key).replace(/[^A-Za-z0-9_-]/g,'_');
                    ...
                    for (var [key, column] of Object.entries(this.config.columns)) {
                        ...
                        if (key == '_data') { // data column

                            // Id gerado com valor "limpo" de caracteres especiais
                            cell.id = this.id + ('_' + content.key).replace(/[^A-Za-z0-9_-]/g,'_') + '_data';
                            cell.classList.add('data');
                            cell.innerText = JSON.stringify(content._data);

                        } else if (key == '_actions' && this.config.table.row_actions) {
        ...
        connectedCallback() {
            ...
            slots_config_columns.addEventListener('slotchange', evt =>  {
                ...
                if (!this.slots_config_columns_processed) {
                    ...
                    // Adiciona a coluna "data" para atributos
                    let cell_data = document.createElement('div');
                    cell_data.classList.add('column');
                    cell_data.classList.add('data');
                    component.config.columns['_data'] = {};
                    header.appendChild(cell_data);
