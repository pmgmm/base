<?php
/**
 * FRAMEWORK - ABSTRACT - WRAPPER SOBRE POSTGRESQL
 * 
 * Disponibiliza métodos que permitem responder a solicitações sobre sobre a DB PostgreSQL
 ...
 * 
 * - setScrollCursor : Informa que a próxima query à db será executada utilizando o cursor em modo SCROLL
 * - unsetScrollCursor : Informa que a próxima query à db será executada utilizando no seu modo default = FWDONLY
 * - resetCursor :  Informa que se pretende iniciar uma nova iteração do result set
 * - closeCursor : Fecha cursor
 ...
 * 
 * - read : Obtém registos
 * - loadNextRecord : Carrega o registo seguinte do result set (ou o primeiro, se reset_cursor = true)
...
abstract class PostgreSQLAbstract {
    ...
    protected bool $reset_cursor;           // Flag de indicação se se pretende fazer o reset do cursor
    protected bool $scroll_cursor;          // Flag de indicação se o cursor será gerado com propriedade SCROLL
    ...
    final private function connect(): bool {
        try {
            ...
            $this->connection = new \PDO ("pgsql:host=$host;port=$port;dbname=$name", $user, $password);
    ...
    /**
     * Informa que a próxima query à db será executada utilizando o cursor em modo SCROLL
     * Este modo permitirá (utilizando o método resetCursor) fazer uma nova iteração do result set
     * 
     * @return void
     * 
     * @pmonteiro (2019-11-18)
     */
    final public function setScrollCursor(): void {
        try {

            $this->scroll_cursor = true;
    ...
    /**
     * Informa que a próxima query à db será executada utilizando no seu modo default = FWDONLY
     * Apenas necessário para reverter o modo SCROLL.
     * 
     * @return void
     * 
     * @pmonteiro (2019-11-18)
     */
    final public function unsetScrollCursor(): void {
        try {

            $this->scroll_cursor = false;
    ...
    /**
     * Informa que se pretende iniciar uma nova iteração do result set
     * 
     * @return void
     * 
     * @pmonteiro (2019-11-18)
     ...
     * @pmonteiro (2019-11-18)
     */
    final public function closeCursor(): void {
        try {

            if ($this->result_set) {
                $this->result_set->closeCursor();
    ...
    final public function read(array $base_table, ?array $fields=null, ?string $conditions=null, ?array $parameters=null, ?array $order=null, ?int $limit=null, ?int $offset=null, ?array $temporaries=null, ?array $joins=null): int {
        try {
            ...
            // Tipo de cursor
            $cursor_option = array(\PDO::ATTR_CURSOR => \PDO::CURSOR_FWDONLY);
            if ($this->scroll_cursor) {$cursor_option = array(\PDO::ATTR_CURSOR => \PDO::CURSOR_SCROLL);}
            ...
            // Preparar statement 
            $this->result_set = $this->connection->prepare($statement, $cursor_option);
            ...
            // Se o tipo de cursor for scroll não é possível conhecer o número de registos abrangidos (devolve false)
            if ($this->scroll_cursor) {$this->result_set_count = false;} 
            else {$this->result_set_count = $this->result_set->rowCount();}
    ...
    /**
     * Carrega o registo seguinte do result set (ou o primeiro, se reset_cursor = true)
     * Ao chegar ao fim dos registos, se o cursor não for do tipo "SCROLL", é destruido.
     * 
    ...
    final public function loadNextRecord(): bool {
        try {
            ...
            if (!$this->reset_cursor) {
                $record = $this->result_set->fetch(\PDO::FETCH_ASSOC, \PDO::FETCH_ORI_NEXT);
            } else {
                $record = $this->result_set->fetch(\PDO::FETCH_ASSOC, \PDO::FETCH_ORI_FIRST);
                $this->reset_cursor = false;
            }
            
            if ((bool)$record) {
                $this->record = $record;
            } elseif ($this->reset_cursor == false) {
                $this->closeCursor();
            }
     ...
     *
     * @param string $sequence = Identificação do campo "..._seq" : schema.table_id_seq
     *
     * @return integer = Último id gerado
     * 
     * @pmonteiro (2019-11-20) 
     */
    final public function getLastGeneratedID(string $sequence): int {
        try {

            $this->result_set = null;   
            $this->last_generated_id = $this->connection->lastInsertId($sequence);
    ...
    /**
     * Devolve e "queima" o id passível de ser utilizado na próxima inserção
     *
     * @param string $sequency = Identificação do campo "..._seq" : schema.table_id_seq
     * 
     * @return integer = Próximo id
     * 
     * @pmonteiro (2019-11-20) 
     */
    final public function getNextID(string $sequence): int {
        try {

            $result = false;
            $result_set = $this->connection->prepare("SELECT nextval('$sequence')");
            $result_set->execute();
            $record = $result_set->fetch(\PDO::FETCH_ASSOC, \PDO::FETCH_ORI_NEXT);
            if ($record) {
                $result = $record['nextval'];  
            }    
