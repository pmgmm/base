...
class AjaxHelper {
	...
	call(settings) {
		...
		// Objecto container de ficheiros
		var obj_action_files = Object.entries('');
...
class AjaxReaderHelper {
     ...
     * @param string|null type = Tipo de  filtro
     * 
     * @pmonteiro (yyyy-mm-dd)
     */
    addField(field, value) {
        let valid = false;
        if ((value.from !== undefined || value.from !== undefined)) {
        ...
        if (valid) {
            if (!this.fields) {this.fields = {}; this.types = {};}
    ...
    addOrder(field, direction) {
        ...
        this.order.push(field + ' ' + direction.trim());
    }
    /**
