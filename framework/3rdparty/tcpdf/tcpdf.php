...
class TCPDF {
	...
	public function _destroy($destroyall=false, $preserve_objcopy=false) {
		...
		if ($destroyall AND !$preserve_objcopy) {
			...
			// TODO: NAVIA : if ($handle = @opendir(rtrim(K_PATH_CACHE,'/'))) {
			// unset($this->imagekeys[array_search($file_name, $this->imagekeys)]);	
			if ($handle = @opendir(rtrim(K_PATH_CACHE,'/'))) {
				while ( false !== ( $file_name = readdir( $handle ) ) ) {
					if (strpos($file_name, '__tcpdf_'.$this->file_id.'_') === 0) {
						unlink(K_PATH_CACHE.$file_name);
						unset($this->imagekeys[array_search(K_PATH_CACHE.$file_name, $this->imagekeys)]);
					}
				}
				closedir($handle);
			}
